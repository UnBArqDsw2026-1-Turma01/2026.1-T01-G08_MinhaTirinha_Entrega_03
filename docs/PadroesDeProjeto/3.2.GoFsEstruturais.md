...
No backend, os Controladores (ComicController e CategoryController) atuam como Facades, expondo interfaces unificadas para as requisições HTTP e delegando a complexidade da lógica de negócio para os serviços (`ComicService` e `CategoryService`).
...
export class CategoryController {
  ...
  getNotRead(@Param('user_id') user_id: string) {
    return this.categoryService.getNotRead(user_id);
  }
}
**Explicação Técnica:** A implementação garantiu que os controladores agissem estritamente como fachadas, removendo qualquer regra de negócio direta dos métodos de rota. [Marjorie Mitzi](https://github.com/Marjoriemitzi).*
...
#### 3.2.3.2.2. Contribuição Implementação 
...
| `1.5` | 22/05/2026 | Atualizando tabela de participação e documentando facade| [Maria Samara](https://github.com/SamaraAlvess) |  [Ana Carolina Madeira Fialho](https://github.com/anawcarol) |
