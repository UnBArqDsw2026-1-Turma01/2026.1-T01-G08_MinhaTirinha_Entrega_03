...
| --- | --- | --- | --- |
| Ana Carolina Madeira Fialho | GoF Estrutural - Facade  | 21/05 | 21:00 |
...
| Maria Samara | GoF Estrutural - Facade/Decorator | 21/05 | 21:00 |
...
### 3.2.3.3. Opiniões dos Participantes
...
### 3.2.3.4. Aplicação do Facade no Backend

Conforme alinhado durante a revisão de código, a arquitetura do backend em NestJS naturalmente favorece a implementação do padrão Facade nos Controladores. A refatoração focou em garantir que o `ComicController` atuasse estritamente como uma interface unificada para as requisições HTTP, delegando toda a complexidade das regras de negócio e integrações (como o Supabase e as estratégias de busca) para o `ComicService`.

#### 3.2.3.4.1. Estrutura Visual (Diagrama de Classes)
...
#### 3.2.3.4.2. Contribuição Individual: Implementação (Marjorie Mitzi)
