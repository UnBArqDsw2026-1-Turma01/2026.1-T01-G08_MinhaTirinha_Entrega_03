...
| Guilherme Flyan | ### | ### | ### |
| João Marcelo Guimarães Costa Naves | GoF Criacional - Factory/Abstract Factory | 07/05 | 14:00 |
| Maria Samara | ### | ### | ### |
| Marjorie Mitzi | ### | ### | ### |
| Pedro Henrique Pereira Santos | GoF Criacional - Factory/Abstract Factory | 07/05 | 14:00 |
| Raissa Silva de Oliveira | GoF Criacional - Factory/Abstract Factory | 07/05 | 14:00 |
| Yasmin Dayrell | ### | ### | ### |
...
![UML Factory](../assets/DesignPatternFactory.drawio.png)
...
O padrão Abstract Factory é um padrão de projeto criacional que define uma interface para criar famílias de objetos relacionados, sem especificar suas classes concretas. No contexto do MinhaTirinha, sua aplicação garante que a criação de histórias — junto com seus elementos visuais e narrativos associados — seja flexível, extensível e desacoplada das implementações concretas, facilitando a adição de novos estilos ou temas de stories sem impactar o código existente.

### 3.1.4.1. Diagrama UML

O padrão GoF Criacional Abstract Factory foi aplicado ao projeto, no seguinte código:

Portanto, assim ficou modelado em UML o padrão Abstract Factory no código do aplicativo.
**Observação:** o arquivo do diagrama completo deve ser referenciado nesta seção assim que o recurso visual estiver disponível.
<font size="2"><p style="text-align: center">Fonte: [João Marcelo Guimarães Costa Naves](https://github.com/JoaoMarceloGCN), [Pedro Henrique Pereira Santos](https://github.com/pedrohpsantos), [Raissa Silva de Oliveira](https://github.com/daisha19), 2026.</p></font>

Baseando-me no diagrama UML fornecido, aqui está o texto reescrito para refletir com precisão a estrutura do Abstract Factory implementada no MinhaTirinha:
...
- **`ITemaTirinhaFactory`**
Interface que define o contrato da Abstract Factory, especificando os métodos `criarPaletaCores(): IPaletaCores` e `criarBalaoFala(): IBalaoFala`. Ela garante que todas as fábricas concretas implementem um mecanismo consistente de criação de **famílias inteiras de objetos relacionados**, sem expor a lógica de instanciação ao cliente.

- **`IPaletaCores` e `IBalaoFala`**
Interfaces que representam os **produtos abstratos** da família. `IPaletaCores` expõe o método `obterCoresPasteis(): List<String>`, enquanto `IBalaoFala` expõe `renderizarEstilo(): void`. Todo o código cliente interage apenas com essas interfaces, sem conhecer as implementações concretas.

- **`TemaTecnologiaFactory` e `TemaInfantilFactory`**
Fábricas concretas que implementam `ITemaTirinhaFactory`, cada uma responsável por criar uma família coesa de produtos. `TemaTecnologiaFactory` instancia `PaletaTecnologia` e `BalaoTecnologia`, enquanto `TemaInfantilFactory` instancia `PaletaInfantil` e `BalaoInfantil` — garantindo que os produtos criados por uma mesma fábrica sejam sempre compatíveis entre si.

- **`PaletaTecnologia`, `PaletaInfantil`, `BalaoTecnologia` e `BalaoInfantil`**
Implementações concretas dos produtos abstratos. Cada classe realiza o comportamento visual específico do seu tema, mas é consumida pelo cliente exclusivamente por meio das interfaces `IPaletaCores` e `IBalaoFala`.

- **`CavaleteScreen`**
Tela que atua como cliente da Abstract Factory, recebendo uma instância de `ITemaTirinhaFactory` via construtor e armazenando os produtos criados (`IPaletaCores` e `IBalaoFala`). O método `configurarAmbiente()` aciona a fábrica para montar o conjunto visual do tema, sem depender de nenhuma classe concreta diretamente.

***

#### Benefícios da Aplicação

**Consistência entre produtos:** como cada fábrica concreta cria todos os objetos de uma mesma família, é impossível misturar, por exemplo, uma `PaletaTecnologia` com um `BalaoInfantil` — a compatibilidade é garantida pela própria estrutura do padrão.

**Baixo acoplamento:** `CavaleteScreen` depende apenas das abstrações `ITemaTirinhaFactory`, `IPaletaCores` e `IBalaoFala`, nunca das classes concretas. A troca de tema exige apenas substituir a fábrica injetada.

**Extensibilidade:** a adição de um novo tema — como `TemaRetroFactory` — exige apenas a criação de uma nova fábrica concreta e seus produtos correspondentes, sem modificar nenhuma classe existente.

**Coesão:** cada classe possui uma responsabilidade única e bem delimitada, separando a lógica de criação (fábricas), a renderização visual (produtos) e a orquestração do ambiente (tela cliente).

Esse modelo demonstra a aplicação estruturada do padrão criacional **Abstract Factory** da GoF ao abstrair a criação de **famílias de objetos visuais interdependentes**, permitindo que o MinhaTirinha suporte múltiplos temas de forma modular, reutilizável e de fácil manutenção.

Aqui estão as opiniões reescritas no contexto do **Abstract Factory**, mantendo a voz e o arco narrativo de cada participante:

***

### 3.1.4.2. Opiniões dos Participantes
...
<details>
  <summary><strong><a href="https://github.com/daisha19">Raissa Silva de Oliveira</a></strong></summary>
  <p>No início, o Abstract Factory parecia excessivo para o que precisávamos. A sensação era de que estávamos criando interfaces, fábricas e produtos demais para simplesmente montar a tela com um tema visual. Parecia abstração pela abstração, sem ganho real. Só que, conforme fui entendendo a proposta, ficou claro que o padrão resolvia um problema que ainda nem tínhamos enfrentado de verdade: a consistência entre os elementos de um mesmo tema. Sem ele, nada impediria que uma paleta de um tema fosse misturada com o balão de fala de outro. Com o Abstract Factory, essa compatibilidade passa a ser garantida pela própria estrutura, sem depender de disciplina manual de quem for adicionar novos temas no futuro.</p>
</details>

<details>
  <summary><strong><a href="https://github.com/JoaoMarceloGCN">João Marcelo Guimarães Costa Naves</a></strong></summary>
  <p>De início, achei que o Abstract Factory era complexidade demais para o escopo do projeto. Criar uma interface de fábrica, duas fábricas concretas, interfaces de produto e quatro implementações só para montar um tema parecia muito esforço para pouca necessidade. Mas isso mudou durante o desenvolvimento. Quando surgiu a possibilidade de adicionar um novo tema, percebi como toda a estrutura já sabia exatamente onde cada coisa deveria entrar. Bastava criar uma nova fábrica concreta com seus produtos correspondentes, e o restante do sistema continuava intocado. A complexidade inicial virou clareza.</p>
</details>

<details>
  <summary><strong><a href="https://github.com/pedrohpsantos">Pedro Henrique Pereira Santos</a></strong></summary>
  <p>No começo, toda aquela hierarquia de interfaces e fábricas para criar objetos visuais me pareceu desnecessária. Parecia muito esforço para algo que poderia ser resolvido de forma mais direta. Só que, durante a implementação, o que antes parecia excesso virou estrutura. Ficou evidente que o padrão não estava apenas organizando a criação dos objetos, mas garantindo que os elementos de cada tema sempre fossem compatíveis entre si. Quando precisamos adicionar um novo tema, toda a alteração ficou concentrada na nova fábrica e nos seus produtos, sem precisar tocar em nada que já estava funcionando.</p>
</details>

### 3.1.3.3. Vídeo Demonstrativo

Foi gravado, na plataforma do Microsoft Teams, uma reunião para a discussão e a modelagem UML do padrão Abstract Factory.
...
| :--: | :--: | :--: | :--: | :--: |
| `1.0` | 07/05/2026 | Adicionando Documentação GoF Criacional Factory | [João Marcelo Guimarães Costa Naves](https://github.com/JoaoMarceloGCN) | [Yasmin Dayrell](https://github.com/YasminDayrell) |
| `1.1` | 08/05/2026 | Adicionando Documentação GoF Criacional Factory | [Raissa Silva de Oliveira](https://github.com/daisha19) | [Ana Carolina Madeira Fialho](https://github.com/anawcarol) |
| `1.2` | 14/05/2026 | Adicionando Documentação GoF Criacional Abstract Factory | [Pedro Henrique Pereira Santos](https://github.com/pedrohpsantos) | [Guilherme Flyan](https://github.com/GFlyan) | 