...
A seguir, apresenta-se a estrutura e as responsabilidades da implementação do padrão Abstract Factory no MinhaTirinha:
...
Interfaces que representam os **produtos abstratos** da família. `IPaletaCores` expõe o método `obterCoresPasteis(): string[]`, enquanto `IBalaoFala` expõe `renderizarEstilo(): void`. Todo o código cliente interage apenas com essas interfaces, sem conhecer as implementações concretas.
...
### 3.1.4.3. Vídeo Demonstrativo

Foi gravado, na plataforma do Microsoft Teams, uma reunião para a discussão e a modelagem UML do padrão Abstract Factory.

A gravação pode ser acessada em: [Vídeo demonstrativo do padrão Abstract Factory](INSIRA_AQUI_O_LINK_DIRETO_DA_GRAVACAO).
