...
O padrão Singleton é um padrão de projeto criacional que garante a existência de uma única instância de uma classe e fornece um ponto global de acesso a ela. 

### 3.1.3.1. Aplicação no Frontend

No frontend do MinhaTirinha, essa instância única é o cliente Supabase exportado pelo módulo `source/mobile/utils/supabase.ts`.
